...
//    NSLog(@"#DEBUG imageRect %.2f %.2f (%.2f %.2f) %@", imgFinalSize.width, imgFinalSize.height, anImage.size.width, anImage.size.height, newModule);
    ...
    titleLabel.text = _module.title;
    titleLabel.font = [self fontWithModuleSize:_module.size];
    
    CGSize newSize = [_module.title sizeWithFont:titleLabel.font constrainedToSize:titleLabel.frame.size];
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {        
        ...
        UITapGestureRecognizer *doubleTapRecognizer = [[[UITapGestureRecognizer alloc] initWithTarget:self
                                                                                              action:@selector(doubleTapReceived:)] autorelease];
